/*=========================================================================
  ...
  Module:    $RCSfile: vtkMyMarchingCubes.cxx,v $
...
vtkCxxRevisionMacro(vtkMyMarchingCubes, "$Revision: 1.1 $");
vtkStandardNewMacro(vtkMyMarchingCubes);

// Description:
// Construct object with initial range (0,1) and single contour value
// of 0.0. ComputeNormal is on, ComputeGradients is off and ComputeScalars is on.
vtkMyMarchingCubes::vtkMyMarchingCubes()
{
...
unsigned long vtkMyMarchingCubes::GetMTime()
{
...
template <class T>
void vtkMyMarchingCubesComputePointGradient(int i, int j, int k, T *s, int dims[3], 
...
//
template <class T>
void vtkMyMarchingCubesComputeGradient(vtkMyMarchingCubes *self,T *scalars, int dims[3], 
                                     ...
                                     int numValues, double dIllegalValThresh)
{
  double s[8], value;
  int i, j, k, sliceSize;
  ...
  for ( k=0; k < (dims[2]-1); k++)
    ...
    for ( j=0; j < (dims[1]-1); j++)
      ...
      for ( i=0; i < (dims[0]-1); i++)
        ...
        if (NeedGradients)
          {
          vtkMyMarchingCubesComputePointGradient(i,j,k, scalars, dims, sliceSize, Spacing, gradients[0]);
          vtkMyMarchingCubesComputePointGradient(i+1,j,k, scalars, dims, sliceSize, Spacing, gradients[1]);
          vtkMyMarchingCubesComputePointGradient(i+1,j+1,k, scalars, dims, sliceSize, Spacing, gradients[2]);
          vtkMyMarchingCubesComputePointGradient(i,j+1,k, scalars, dims, sliceSize, Spacing, gradients[3]);
          vtkMyMarchingCubesComputePointGradient(i,j,k+1, scalars, dims, sliceSize, Spacing, gradients[4]);
          vtkMyMarchingCubesComputePointGradient(i+1,j,k+1, scalars, dims, sliceSize, Spacing, gradients[5]);
          vtkMyMarchingCubesComputePointGradient(i+1,j+1,k+1, scalars, dims, sliceSize, Spacing, gradients[6]);
          vtkMyMarchingCubesComputePointGradient(i,j+1,k+1, scalars, dims, sliceSize, Spacing, gradients[7]);
          }
		...
		index |= CASE_MASK[ii];
	      ...
	      int iUsedIllegalPoint = 0;

            for (ii=0; ii<3; ii++) //insert triangle
              {
              vert = edges[edge[ii]];

	      if ( (s[vert[0]]<dIllegalValThresh ) || (s[vert[1]]<dIllegalValThresh) ) {
		iUsedIllegalPoint = 1;
	      }
            ...
            if ( ptIds[0] != ptIds[1] &&
                 ptIds[0] != ptIds[2] &&
                 ptIds[1] != ptIds[2] && !iUsedIllegalPoint )
                {
...
// Contouring filter specialized for volumes and "short int" data values.  
//
int vtkMyMarchingCubes::RequestData(
...
//
  ...
  if (inScalars->GetNumberOfComponents() == 1 )
    ...
    switch (inScalars->GetDataType())
      {
      vtkTemplateMacro(
        vtkMyMarchingCubesComputeGradient(this, static_cast<VTK_TT*>(scalars),
                                        dims,origin,Spacing,this->Locator,
                                        newScalars,newGradients,
                                        newNormals,newPolys,values,
                                        numContours,dIllegalValThresh)
        );
      } //switch
    }
    ...
    vtkMyMarchingCubesComputeGradient(this,scalars,dims,origin,Spacing,this->Locator,
                  newScalars,newGradients,
                  newNormals,newPolys,values,numContours,dIllegalValThresh);
    image->Delete();
    }
...
void vtkMyMarchingCubes::SetLocator(vtkPointLocator *locator)
{
...
void vtkMyMarchingCubes::CreateDefaultLocator()
{
  if ( this->Locator == NULL)
    {
...
int vtkMyMarchingCubes::FillInputPortInformation(int, vtkInformation *info)
{
  info->Set(vtkAlgorithm::INPUT_REQUIRED_DATA_TYPE(), "vtkImageData");
  return 1;
}

void vtkMyMarchingCubes::PrintSelf(ostream& os, vtkIndent indent)
{
